...
def test_windows_old_is_report_only_and_has_no_whole_root_authority() -> None:
    ...
    assert _root_policy("windows-old") is CleanupPolicy.REPORT_ONLY
...
def test_mixed_windows_maintenance_roots_are_not_universal_delete_targets() -> None:
    assert _root_policy("windows-maintenance") is CleanupPolicy.REPORT_ONLY
    assert _root_policy("windows-update-downloads") is CleanupPolicy.REPORT_ONLY


def test_system_crash_dumps_are_report_only() -> None:
    assert _root_policy("system-crash-dumps") is CleanupPolicy.REPORT_ONLY


def test_user_model_store_is_not_a_vendor_managed_cache() -> None:
    assert _root_policy("lmstudio") is CleanupPolicy.REPORT_ONLY


def test_broad_unowned_cache_parents_do_not_receive_deterministic_authority() -> None:
    assert _root_policy("ide-working-caches") is CleanupPolicy.REPORT_ONLY
    assert _root_policy("general-tool-caches") is CleanupPolicy.REPORT_ONLY

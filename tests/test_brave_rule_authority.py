...
def test_ai_cannot_delete_brave_profile_cache_storage_or_updater_state(
    tmp_path: Path,
...
def test_ai_can_learn_brave_http_cache_and_updater_staging_decisions(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _environment(tmp_path, monkeypatch)
    ...
    assert updated.decision_for(staging) is RuleDecision.DELETE

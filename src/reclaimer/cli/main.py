"""Reclaimer command-line entry point."""
...
from reclaimer.adapters.base import AdapterContext, InventoryAdapter
from reclaimer.adapters.catalog import get_descriptor
from reclaimer.adapters.command import run_query
from reclaimer.adapters.conda import CondaCacheAdapter
from reclaimer.adapters.docker import DockerAdapter
from reclaimer.adapters.huggingface import HuggingFaceAdapter
from reclaimer.adapters.npm import NpmCacheAdapter
from reclaimer.adapters.ollama import OllamaAdapter
from reclaimer.adapters.pip_cache import PipCacheAdapter
from reclaimer.adapters.pnpm import PnpmStoreAdapter
from reclaimer.adapters.uv_cache import UvCacheAdapter
from reclaimer.adapters.vscode import VSCodeExtensionAdapter
...
def _elevated_main_process() -> bool:
    ...
    if elevated:
        print(
            "Reclaimer's main process must run without elevation. "
...
def _guides(*, as_json: bool) -> int:
    ...
    payload = {
        ...
        "safety_boundary": (
            "Reclaimer does not execute, elevate, journal, verify, or audit this command."
        ),
    }
    if as_json:
    ...
    try:
        with StateStore() as store:
            ...
            if scan["status"] != ScanStatus.COMPLETED.value:
                ...
                return 1
            records = store.get_resources_by_ids(args.scan_id, args.select)

...
Windsurf is VS Code/Electron-derived, while Codeium/Windsurf AI and index state
can live beside editor state and under shared home directories. DevClean only
delegates well-understood runtime caches, logs, and crash reports to generic
cleanup; unknown AI/index/application state remains protected.
"""
...
WINDSURF_RULES: tuple[ApplicationCleanupRule, ...] = (
    ...
    _tool_dir(
        "windsurf-cached-extension-vsix",
    ...
    _rule(
        ...
        DecisionOwner.USER,
        RebuildCost.HIGH,
        "Windsurf workspace-local editor/AI state",
    ...
    _rule(
        ...
        DecisionOwner.KEEP,
        RebuildCost.HIGH,
        "Windsurf unsaved editor / recovery data",
    ),
    _rule(
        "windsurf-user-state",
        "User",
        ...
        DecisionOwner.KEEP,
        RebuildCost.HIGH,
        "Windsurf settings, global storage, extension state, and AI state",
    ),
    _rule(
        "windsurf-extension-root",
        "",
        MatchKind.PREFIX,
        DecisionOwner.KEEP,
        RebuildCost.HIGH,
    ...
    _rule(
        ...
        DecisionOwner.KEEP,
        RebuildCost.HIGH,
        "Windsurf/Codeium home configuration, AI, and index state",
        root_kind="home",
    ),
    _rule(
        "windsurf-unknown-state",
        "",
        MatchKind.PREFIX,
        DecisionOwner.KEEP,
        RebuildCost.HIGH,
        "Unclassified Windsurf application state",
    ),
)
...
def windsurf_roots(environment: Mapping[str, str] | None = None) -> WindsurfRootSet:
    ...
    explicit_data = env.get("windsurf_user_data_dir")
    explicit_extensions = env.get("windsurf_extensions_dir")
    if explicit_data:
    ...
    if appdata:
        data.append(PureWindowsPath(appdata) / "Windsurf")
    if localappdata:
        data.append(PureWindowsPath(localappdata) / "Windsurf")
    if profile:
        windsurf_home = PureWindowsPath(profile) / ".windsurf"
        codeium_home = PureWindowsPath(profile) / ".codeium"
        homes.extend((windsurf_home, codeium_home))
        extensions.append(windsurf_home / "extensions")
...
def windsurf_scan_roots(
    ...
    return windsurf_roots(environment).data_roots
...
def match_windsurf_rule(
    ...
    groups = {
        ...
        "WINDSURF_HOME": roots.home_roots,
    }
...
def windsurf_audited_tool_roots(
    ...
    for rule in WINDSURF_RULES:
        if rule.owner is not DecisionOwner.TOOL or not rule.allow_whole_tree:
            continue
        if rule.root_key != "WINDSURF_DATA":
            continue
...
def windsurf_process_running() -> bool:
    if os.name != "nt":
        return False
    script = (
        "$p=Get-CimInstance Win32_Process | Where-Object { $_.Name -ieq 'Windsurf.exe' }; "
        "if ($p) { 'RUNNING' }"
    )
    try:

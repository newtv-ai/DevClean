...
def prune_wsl_pnpm_store(
    ...
    """Run pnpm's own GC for the freshly verified local-rootfs store identity."""
    ...
    # Vendor ownership is not sufficient locality proof. A pnpm store can be
    # redirected to /mnt/c, a network mount, removable storage, or another
    # separately mounted filesystem. Bind both the active versioned store and
    # the store-dir used for the mutation to the selected distro root filesystem
    # immediately before the vendor prune command.

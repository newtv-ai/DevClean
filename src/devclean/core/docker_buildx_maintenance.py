...
def list_buildx_builders(
    ...
    """List exact Buildx builders and classify only proven-local nodes executable."""
...
def inspect_buildx_cache(
    ...
    *,
    ...
    """Inventory Buildx records that are both old enough and reclaimable."""
...
def _inspect_builder(
    ...
    try:
        ...
        raise RuntimeError("无法解析 docker buildx inspect formatter JSON") from error
    if not isinstance(payload, dict):
        raise RuntimeError("docker buildx inspect formatter 未返回 JSON object")

    reported_name = _required_string(payload, "Name", "Buildx inspect")
    if reported_name != builder_name:
        raise RuntimeError(
            "docker buildx inspect 返回了意外 builder 身份: "
            f"requested={builder_name}, reported={reported_name}"
        )
    driver = _required_string(payload, "Driver", "Buildx inspect")
    nodes = _nodes_from_payload(payload.get("Nodes"))
    ...
    if not nodes:
        return BuildxBuilder(
            ...
            local=False,
            executable=False,
            reason="Buildx inspect 未返回可验证 node endpoint; 仅报告",
        )
    if driver not in {"docker", "docker-container"}:
        return BuildxBuilder(
            name=reported_name,
            driver=driver,
            nodes=nodes,
...
def _nodes_from_payload(value: object) -> tuple[BuildxNode, ...]:
    if value is None:
        return ()
    if not isinstance(value, list):
        raise RuntimeError("Buildx inspect Nodes 不是 JSON array")
    nodes: list[BuildxNode] = []
    for item in value:
        if not isinstance(item, dict):
            raise RuntimeError("Buildx inspect Nodes 包含非 object")
        name = _required_string(item, "Name", "Buildx node")
        endpoint = _required_string(item, "Endpoint", "Buildx node")
        status = str(item.get("Status", "")).strip()
        nodes.append(BuildxNode(name=name, endpoint=endpoint, status=status))
    return tuple(nodes)
...
def _buildx_du_records(
    ...
    for line in result.stdout.splitlines():
        ...
        record_id = _required_string(payload, "ID", "Buildx du")

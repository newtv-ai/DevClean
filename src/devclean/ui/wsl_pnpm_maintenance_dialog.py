...
class _WslPnpmMaintenanceDialog:
    def __init__(self, parent: tk.Tk | tk.Toplevel) -> None:
        ...
        explanation = ttk.LabelFrame(container, text="清理语义与存储边界", padding=10)
        explanation.pack(fill=tk.BOTH, expand=True, pady=(10, 0))
        ttk.Label(
            explanation,
            text=(
                "pnpm 官方将 `store prune` 定义为删除没有被系统中任何已登记项目引用的包. "
                "官方同时说明该操作不会伤害现有项目,没有项目副作用; 以后若再次需要被"
                "移除的包,pnpm 会重新下载.\n\n"
                "因此这里属于 DETERMINISTIC_CANDIDATE,不送 AI. 但官方也建议偶尔运行"
                "而不是过于频繁,因为切换分支或恢复旧依赖时可能再次需要那些包.\n\n"
                "执行前 DevClean 会重新确认 pnpm 版本、active store、store-dir 和运行状态,"
                "并要求 active store 与 store-dir 都位于所选发行版的根文件系统. 如果缓存"
                "被重定向到 /mnt/c、网络盘、可移动盘或其他独立挂载点,这里只报告而不执行.\n\n"
                "真正删除由 pnpm 自己完成. WSL 2 中释放 Linux 逻辑空间并不等于 Windows "
                "侧虚拟磁盘文件同步缩小."
            ),
            wraplength=870,
            justify=tk.LEFT,
    ...
    def _start_pnpm_inventory(self) -> None:
        ...
        if not distro.running and not messagebox.askyesno(
            "WSL pnpm Store 维护",
            "所选发行版当前已停止. 检查 pnpm 需要在该发行版内执行命令, "
            "因此可能启动它. 是否继续?",
        ):
            return
    ...
    def _start_prune(self) -> None:
        ...
        if not messagebox.askyesno(
            ...
            "DevClean 将重新确认这些身份、pnpm 运行状态和本地根文件系统范围,然后固定 "
            "store-dir 执行 `pnpm store prune`. 继续吗?",
        ):
            return

        self._set_busy(True)
        self._status.set("正在重新确认 pnpm/store 身份、运行状态和文件系统范围...")
    ...
    def _handle_pnpm(self, event: _PnpmEvent) -> None:
        ...
        self._details.configure(
            text=(
                f"待执行前本地范围复核 | {inventory.distribution} ({state})\n"
                f"pnpm 版本: {inventory.version_text}\n"
                f"active store: {inventory.active_store_path}\n"
                f"执行时固定 store-dir: {inventory.store_dir}"
            )
        )
        self._status.set("已确认 pnpm store 语义. 真正执行前还会重新验证本地文件系统范围.")
        self._set_busy(False)

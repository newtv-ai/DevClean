...
class _WslPnpmMaintenanceDialog:
    def __init__(self, parent: tk.Tk | tk.Toplevel) -> None:
        ...
        ttk.Label(
            ...
            text=(
                ...
                "DevClean 会在执行前重新确认 pnpm 版本、active store 和 store-dir,并检查"
                "当前发行版内没有 pnpm 操作. 真正删除由 pnpm 自己完成. WSL 2 中释放"
                "Linux 逻辑空间并不等于 Windows 侧虚拟磁盘文件同步缩小."
            ),
            wraplength=870,
            justify=tk.LEFT,
        ).pack(anchor=tk.W)
    ...
    def _start_prune(self) -> None:
        ...
        if not messagebox.askyesno(
            ...
            "DevClean 将重新确认这些身份和 pnpm 运行状态,然后固定 store-dir 执行 "
            "`pnpm store prune`. 继续吗?",
        ):
            return

        self._set_busy(True)
        self._status.set("正在重新确认 pnpm/store 身份和运行状态...")

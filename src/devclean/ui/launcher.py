...
def _install_tools_menu(root: tk.Tk) -> None:
    ...
    tools.add_command(label="Codex 历史管理…", command=lambda: open_codex_history_dialog(root))
    tools.add_command(
        label="Claude Code 存储维护…", command=lambda: open_claude_maintenance_dialog(root)
    )
    tools.add_command(label="Cursor 存储维护…", command=lambda: open_cursor_maintenance_dialog(root))
    tools.add_command(label="VS Code 存储维护…", command=lambda: open_vscode_maintenance_dialog(root))
    tools.add_command(
        label="JetBrains 过期版本存储维护…",
        command=lambda: open_jetbrains_leftover_maintenance_dialog(root),
    )
    tools.add_separator()
    tools.add_command(label="NuGet 缓存维护…", command=lambda: open_nuget_maintenance_dialog(root))
    tools.add_command(label="pip 缓存维护…", command=lambda: open_pip_maintenance_dialog(root))
    tools.add_command(label="pnpm Store 垃圾收集…", command=lambda: open_pnpm_maintenance_dialog(root))
    tools.add_command(label="uv 缓存垃圾收集…", command=lambda: open_uv_maintenance_dialog(root))
    tools.add_command(label="Go 缓存维护…", command=lambda: open_go_maintenance_dialog(root))
    tools.add_command(label="Conda 安全缓存维护…", command=lambda: open_conda_maintenance_dialog(root))
    tools.add_command(label="Conan 2 安全缓存维护…", command=lambda: open_conan_maintenance_dialog(root))
    tools.add_command(label="vcpkg 存储维护…", command=lambda: open_vcpkg_maintenance_dialog(root))
    tools.add_command(
    ...
    tools.add_command(label="Unreal DDC 安全维护…", command=lambda: open_unreal_maintenance_dialog(root))
    tools.add_command(label="Bazel 工作区维护…", command=lambda: open_bazel_maintenance_dialog(root))
    tools.add_command(
        label="Cargo 工作区 target 维护…", command=lambda: open_cargo_project_maintenance_dialog(root)
    )
    tools.add_command(
        label="Meson 构建目录维护…", command=lambda: open_meson_project_maintenance_dialog(root)
    )
    tools.add_command(
    ...
    tools.add_command(
        label="Podman 已停止容器维护…", command=lambda: open_podman_container_maintenance_dialog(root)
    )
    tools.add_command(label="Podman 镜像维护…", command=lambda: open_podman_image_maintenance_dialog(root))
    tools.add_command(
        label="Unity 项目 Library 维护…", command=lambda: open_unity_project_maintenance_dialog(root)
    )
    tools.add_command(
    ...
    tools.add_command(
        label="Unity UPM 全局缓存维护…", command=lambda: open_unity_upm_maintenance_dialog(root)
    )
    tools.add_separator()
    tools.add_command(
    ...
    tools.add_separator()
    tools.add_command(label="WSL 发行版存储概览…", command=lambda: open_wsl_inventory_dialog(root))
    tools.add_command(label="WSL pip 缓存维护…", command=lambda: open_wsl_pip_maintenance_dialog(root))
    tools.add_command(label="WSL uv 缓存维护…", command=lambda: open_wsl_uv_maintenance_dialog(root))
    tools.add_command(
        label="WSL pnpm Store 维护…", command=lambda: open_wsl_pnpm_maintenance_dialog(root)
    )
    tools.add_command(
        label="WSL Go 构建缓存维护…", command=lambda: open_wsl_go_build_cache_dialog(root)
    )
    tools.add_command(
        label="WSL Go 模块缓存维护…", command=lambda: open_wsl_go_mod_cache_dialog(root)
    )

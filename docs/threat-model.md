...
本模型覆盖当前仓库已经实现的 Windows inventory：普通权限下扫描显式本地根、运行九个内置只读适配器（Hugging Face、pip、uv、Conda、npm、pnpm、Docker、Ollama、VS Code）、保存本地状态与证据、导出报告，以及为已保存候选生成不可执行的 `REPORT_ONLY` review plan。Windows Component Store 仅提供由用户自行复制执行的官方命令指引；Reclaimer 不运行该命令。

当前范围没有删除、移动、回收站、厂商 GC、维护、提权、服务启动、运行时插件、BleachBit 调用或 AI 决策。`plan create/show` 不是执行能力：计划中的每个动作都必须为 `REPORT_ONLY`、`enabled=false`，计划本身必须为 `executable=false`，并且 CLI 不存在 `apply` 或 `execute` 命令。
...
不存在：管理员 broker、cleanup executor、BleachBit 运行路径、运行时插件、
云端服务、远程 Ollama endpoint 或 AI 控制面。
```
...
当前通过“不实现执行”规避以下风险，不能视为已经解决：可执行 plan 的 TTL/preflight、持久 intent 与 crash reconcile、按句柄删除、批准根与用户资产 deny-list、稳定 file ID 重核验、锁与并发、厂商 GC 动态候选集、部分成功、不可逆确认、逻辑释放量与宿主物理释放量差异，以及最小权限签名 broker 的安装、IPC、ACL 与 DLL 搜索路径。
...
因此当前不得增加 cleanup execution、direct filesystem delete、broker action 或 BleachBit `--clean` 路径。任何实现上述能力的 PR 必须先为对应风险提供设计、失败模式、端到端证据与门槛更新。

## 9. 发布门槛与当前结论

v0.1 对外发布必须同时满足方案中的 G0、G1 与 G2，而本地单机成功不能替代外部门槛：
...
截至 2026-07-11，仅有单机本地实现和部分证据；本文件不声明 G0、G1、G2、G5 或 G6 已通过，也不授权发布清理能力。

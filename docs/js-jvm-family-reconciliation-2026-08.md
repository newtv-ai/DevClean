...
This pass closes the queued JavaScript-package-manager/test-browser family and JVM family on the current post-#158 `main`. It is a reconciliation pass, not a new policy rewrite: each named product already has a source-audited storage or maintenance contract with regression coverage, and later corrective PRs removed broad raw deletion where vendor lifecycle could not support it.

## JavaScript family

The family is re-verified as the combination of the following current source-backed lanes and protections:

- npm — #134 replaces broad raw cache deletion with exact npm-owned maintenance: `npm cache verify` for vendor GC, explicit USER_REVIEW for full package-cache clear and exact npx-key removal, while `_tuf`, config, prefix and unknown root state stay protected. A later diagnostic-log second pass moves npm logs to USER_REVIEW rather than age/size-based raw TOOL cleanup.
- pnpm — #57 introduced `pnpm store prune` as the vendor garbage collector and protected the raw content-addressable/global store. The 2026-08-23 second pass tightens that contract against current pnpm 11 source: the command also performs DLX expiry and orphaned-global-install cleanup, so DevClean now isolates those secondary scopes into a verified temporary sandbox before prune. Raw `<cache>/dlx` and metadata mirrors are USER_REVIEW rather than age-based whole-tree TOOL; exact `pnpm-state.json` update-check state is deterministic TOOL when pnpm is idle. See `docs/pnpm-cache-state-prune-reaudit.md`.
...
Cross-family conclusion: no cache-looking directory name, version folder, age, size, or learned rule is sufficient to create raw JavaScript-family deletion authority. Positive mutation paths are vendor commands whose exact executable/root/scope is revalidated; ambiguous or configuration-sensitive stores remain protected/report-only or USER_REVIEW as the source evidence requires.
...
The cited implementation PRs carry product-level regression tests for root discovery, KEEP/REPORT_ONLY projection, whole-tree authority, process/configuration guards, learned-rule ceilings, and vendor-maintenance revalidation. Later second-pass corrections supersede older summaries where current vendor source changed or exposed a wider command scope. Every implementation-changing PR still must pass the repository-wide lock/dependency checks, Ruff, strict mypy, full pytest/current CI, Windows EXE artifact build, and CodeQL before merge.

## Revisit triggers

Re-open a family when vendor documentation/source materially changes the cleanup scope, introduces a bounded machine-readable destructive manifest, removes a currently relied-upon safety warning, or changes an executable/cache-root/configuration resolution contract. A new cache-like path or age/size heuristic alone is not a revisit reason.

...
1. Reclaimer 是独立的 Python 引擎，不 fork、不 import、不复制 BleachBit 源码或内置 cleaner 定义。
2. Phase 0 和 v0.1 只做 inventory，不调用任何 BleachBit 清理动作。
...
- 独立引擎可以先验证扫描、Schema 和安全不变量，再决定是否需要承担上游 GUI/核心代码的维护成本。

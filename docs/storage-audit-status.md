...
| Brave Update storage | REPORT_ONLY vendor-managed updater lifecycle; install working tree and diagnostic log have no generic age/size or whole-tree delete authority |
| Vivaldi Crashpad reports | REPORT_ONLY diagnostic evidence; no seven-day/whole-tree raw deletion, while the owning Crashpad report database retains lifecycle authority |
| Opera System Cache | REPORT_ONLY/browser-managed; no generic 14-day/8-MiB or whole-tree raw deletion authority |
...
| Brave Update install/log storage | generic raw authority removed; Omaha owns exact install-working-directory clearing, package-cache GC and log size lifecycle while DevClean's old rule used broader environment roots plus independent age/size thresholds | dedicated source-bound contents-only install-working-directory lane with exact Windows path identity, updater concurrency proof and fresh revalidation |
| Vivaldi Crashpad reports | generic raw authority removed; Vivaldi documents reports as crash/bug-report evidence and current Crashpad owns report-database pruning rather than a seven-day whole-tree filesystem lifecycle | exact Vivaldi/Crashpad vendor lifecycle/API or a dedicated exact-report USER_REVIEW lane with database identity, concurrency and fresh revalidation proof |
| Opera System Cache | generic raw authority removed; current Opera Help supports browser cache clearing but does not define the former 14-day raw `System Cache` filesystem lifecycle, while community troubleshooting deletion is not a bounded cleaner contract | exact Opera API/source-backed cache object lifecycle or safely scoped browser cache-clear operation with exact profile/root binding and fresh revalidation |
...
A full rule re-audit is active in `docs/full-rule-reaudit-2026-08.md`. #142 fixed default-backup/migration handling but temporarily neutralized packaged learned rules more aggressively than the product policy requires. Phase 2 restores the intended boundary: confirmed learned/default DELETE/KEEP knowledge is reusable for files, never for directories, while generic name/suffix/category/unknown evidence alone routes to protection instead of asking a non-expert user or automatically spending paid AI calls. Static VENDOR_MANAGED fallback is now closed: static scan roots are discovery-only and runtime vendor authority requires an attached audited application TOOL whole-tree rule. Continue with AGE_BASED_REVIEW lifecycle, selective file-default restoration, then application modules one by one.
...
1. **High-impact application caches**
   - continue auditing known applications individually; npm/Cypress/Hugging Face are vendor-aware, while PyTorch Hub, Yarn, Bun, Gradle User Home, Brave Update, Vivaldi Crashpad and Opera System Cache are explicitly protected where the current generic lane cannot reproduce the bounded vendor lifecycle.
